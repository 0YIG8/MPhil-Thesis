...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    // Set up moving mesh
    SRFMesh SRF_mesh(fileLocations.lookup("points"), fileLocations.lookup("faces"), fileLocations.lookup("cells"), fileLocations.lookup("boundaries"), origin);
    
    // Set omega for all cells
    SRF_mesh.setOmegaForAllCells(omega);

    // Get list of cells, faces, points
    std::vector<Cell> myListOfAllCls = SRF_mesh.getListOfCells(); // Get list of cells in mesh
    std::vector<Face> myListOfAllFcs = SRF_mesh.getListOfFaces(); // Get list of all faces in the mesh
    ...
    /*
        MAIN CALC - Eg. SIMPLE
    */
    evaluateSIMPLE(SRF_mesh, velocity, pressure, root);

    // outputVTK_SRFData(SRF_mesh, pressure, velocity, 0, "mixerSRF/VTK/");

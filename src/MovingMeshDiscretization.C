        ...
        if (f->getIfBoundaryFace() == true) // For boundary faces
        {
            for (size_t k = 0; k < fieldVariable.nameOfBoundaryPatch.size(); k++)
            {
                if (f->getBoundaryName() == fieldVariable.nameOfBoundaryPatch[k])
                {
                    ...
                    else if (fieldVariable.typeOfBoundaryCondition[k] == BoundaryType::velocitySRF)
                    {
                        Vector boundaryValue = omega.crossMult(f->getFaceCentre() - origin) * -1.0;
                        fieldVariable.boundaryValueX[k] = boundaryValue[0];
                        fieldVariable.boundaryValueY[k] = boundaryValue[1];
                        fieldVariable.boundaryValueZ[k] = boundaryValue[2];
                    ...
                    else if (fieldVariable.typeOfBoundaryCondition[k] == BoundaryType::velocityMRF)
                    {
                        Vector boundaryValue = omega.crossMult(f->getFaceCentre() - origin);
                        fieldVariable.boundaryValueX[k] = boundaryValue[0];
                        fieldVariable.boundaryValueY[k] = boundaryValue[1];
                        fieldVariable.boundaryValueZ[k] = boundaryValue[2];
...
void evaluateConvectionTerm(std::vector<Face>& listOfAllFaces, VectorField& fieldVariable, VectorField& velocityField, ConvectionScheme scheme, Vector omega, Vector origin)
{
    for (size_t i = 0; i < listOfAllFaces.size(); i++)
    {
        Face* f = &listOfAllFaces[i];
        ...
        if (f->getIfBoundaryFace() == true) // For boundary faces
        {
            for (size_t k = 0; k < fieldVariable.nameOfBoundaryPatch.size(); k++)
            {
                if (f->getBoundaryName() == fieldVariable.nameOfBoundaryPatch[k])
                {
                    ...
                    else if (fieldVariable.typeOfBoundaryCondition[k] == BoundaryType::velocitySRF) // Only for velocity field
                    {
                        Vector boundaryValue = omega.crossMult(f->getFaceCentre() - origin) * -1.0;
                        fieldVariable.boundaryValueX[k] = boundaryValue[0];
                        fieldVariable.boundaryValueY[k] = boundaryValue[1];
                    ...
                    else if (fieldVariable.typeOfBoundaryCondition[k] == BoundaryType::velocityMRF) // Only for velocity field
                    {
                        Vector boundaryValue = omega.crossMult(f->getFaceCentre() - origin);
                        fieldVariable.boundaryValueX[k] = boundaryValue[0];
                        fieldVariable.boundaryValueY[k] = boundaryValue[1];
...
double getFlux(Face* f, std::vector<Cell>& listOfAllCells, VectorField& velocityField, int& idxP, int& idxN, Vector omega, Vector origin, bool isMRF) // For vector field velocity (interpolated u)
{
    double fx = f->getFaceInterpolationFactor();
    ...
    if (f->getIfBoundaryFace() == true)  // If the face is a boundary
    {
        for (size_t k = 0; k < velocityField.nameOfBoundaryPatch.size(); k++)
        {
            if (f->getBoundaryName() == velocityField.nameOfBoundaryPatch[k])
            {
                ...
                else if (velocityField.typeOfBoundaryCondition[k] == BoundaryType::velocitySRF)
                {
                    Vector boundaryValue = omega.crossMult(f->getFaceCentre() - origin) * -1.0;

                    vel_X = boundaryValue[0];  
                    vel_Y = boundaryValue[1];
                    vel_Z = boundaryValue[2];
                }
                else if (velocityField.typeOfBoundaryCondition[k] == BoundaryType::velocityMRF)
                {
                    Vector boundaryValue = omega.crossMult(f->getFaceCentre() - origin);
    ...
    // Calculate flux
    if (isMRF == true) // if its an MRF simulation and the face is part of a cell in the moving zone
    {
        // Old version
        flux = (f->getFaceNormal() * f->getFaceArea()).dotMult(vel) - (f->getFaceNormal() * f->getFaceArea()).dotMult(omega.crossMult(f->getFaceCentre() - origin));
    }
    else // if not an MRF simulation (i.e. its SRF)
    {
        flux = (f->getFaceNormal() * f->getFaceArea()).dotMult(vel);
    }
...
void initializeFlux(std::vector<Face>& listOfFaces, std::vector<Cell>& listOfAllCells, VectorField velocity, Vector omega, Vector origin, bool isMRF)
{
    ...
    {
        Face* f = &listOfFaces[i];
        int idxP = f->getFaceOwner();
        int idxN = f->getFaceNeighbour();

        double F = getFlux(f, listOfAllCells, velocity, idxP, idxN, omega, origin, isMRF);
        f->setFaceFlux(F);
    }
}
